...
public class Kassa {
    ...
    public void printNota(){
        ...
        for (Produk produk: pesanan.keySet()){
            ...
            total += subtotal;
            System.out.println(c + ".  " + produk.getNama() + " "+ produk.getNama().length() + pesanan.get(produk) + "x" + " "+String.valueOf(pesanan.get(produk)).length() + produk.getHarga() + "    " +  subtotal);
            c += 1;
        }
    ...
    public void printPenjualanKasir(){
        ...
        int i = 1;
        for (Kasir kasir: arrKasir){
            System.out.println(i + ".   " + kasir.getNama() + "        "+kasir.getNama().length() +"              "+ "Rp" + kasir.getTotalPenjualan());
            i += 1;
        }
    }
}
